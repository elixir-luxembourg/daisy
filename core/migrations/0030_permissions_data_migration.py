...
def is_test_environment(db_cursor):
    db_cursor.execute("SELECT id from django_content_type " "WHERE app_label='core'")
    ...
    with connection.cursor() as cursor:
        ...
        if results:
            for id, codename, content_type_id, model_name in results:
                new_codename = (
                    f"{codename if codename != 'edit' else 'change'}_{model_name}"
                )
                ...
                if not new_permission_id:
                    if codename in ["admin", "protected"] and model_name not in [
                        "contract",
                        "dataset",
                        "project",
                    ]:
                        ...
                        raise ValueError(
                            f"No permission was found for codename={new_codename} and content_type_id={content_type_id}"
                        )
                else:
                    assert len(new_permission_id) == 1
                    (new_permission_id,) = new_permission_id[0]
                    print(
                        f"Replacing new permission {new_codename} (id={new_permission_id}) by {codename} (id={id}, target={model_name})"
                    )
                    cursor.execute(
            ...
            raise DataError(
                "Old permissions were removed from the database. Impossible to roll back"
            )
    ...
    with connection.cursor() as cursor:
        ...
        if results:
            for id, codename, content_type_id, model_name in results:
                ...
                previous_codename = (
                    "edit" if previous_codename == "change" else previous_codename
                )
                ...
                if not old_permission_id:
                    raise ValueError(
                        f"No permission was found for codename={previous_codename} and content_type_id={content_type_id}"
                    )
                else:
                    assert len(old_permission_id) == 1
                    (old_permission_id,) = old_permission_id[0]
                    print(
                        f"Replacing new permission {codename} (id={id}, target={model_name}) by {previous_codename} (id={old_permission_id})"
                    )
                    cursor.execute(
        ...
        if results:
            ...
            for id, codename in results:
                ...
                if not old_permission_id:
                    ...
                    (old_permission_id,) = old_permission_id[0]
                    print(
                        f"Replacing old permission {codename} (id={id}, target={model_name}) by {previous_codename} (id={old_permission_id})"
                    )
                    cursor.execute(
        ...
        if results:
            for id, codename, content_type_id, model_name in results:
                ...
                if not new_permission_id:
                    ...
                    (new_permission_id,) = new_permission_id[0]
                    print(
                        f"Replacing old permission {codename} (id={id}, target={model_name}) by {new_codename} (id={new_permission_id})"
                    )
                    cursor.execute(
                        "UPDATE core_user_user_permissions "
                        "SET permission_id=%s "
...
def update_group_permissions(apps, schema_editor):
    with connection.cursor() as cursor:
        ...
        for group, permissions_dict in GROUP_PERMISSIONS.items():
            # Find the group id
            cursor.execute(
                "SELECT id from auth_group " "WHERE auth_group.name=%s", [group.value]
            )
            ...
            try:
                ...
                raise DataError(
                    f"More than one group named {group.value} was found in database"
                )
            ...
            for model_label, permissions_list in permissions_dict.items():
                ...
                for perm in permissions_list:
                    ...
                    if not perm_id:
                        raise DataError(
                            f"Permission {perm} (content_type={model_name}) was not found in database"
                        )
                    ...
                    try:
                        ...
                        raise DataError(
                            f"More than one permission {perm} (content_type={model_name}) was found in database"
                        )
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunPython(
            update_guardian_userobjectpermissions,
            rollback_guardian_userobjectpermissions,
        ),
        migrations.RunPython(
            update_core_datasetuserobjectpermissions,
            rollback_core_datasetuserobjectpermissions,
        ),
        migrations.RunPython(
            update_core_projectuserobjectpermissions,
            rollback_core_projectuserobjectpermissions,
        ),
