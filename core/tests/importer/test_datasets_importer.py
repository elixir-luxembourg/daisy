...
@pytest.mark.django_db
def test_import_datasets(celery_session_worker, storage_resources, partners, gdpr_roles, can_defer_constraint_checks):
    ...
    assert ["Igor Teal"] == [custodian.full_name for custodian in d1.local_custodians.all()]
    ...
    assert ["Joanne Swift"] == [employee.full_name for employee in d2.local_custodians.all()]
    ...
    assert ["Rob Blue"] == [employee.full_name for employee in d3.local_custodians.all()]
