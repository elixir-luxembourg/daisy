...
class Command(BaseCommand):
    ...
    def create_gdpr_roles():
        ...
        with open(os.path.join(FIXTURE_DIR, "gdpr-roles.json"), "r") as handler:
            ...
            for gdpr_role in data:
                GDPRRole.objects.get_or_create(**gdpr_role)
    ...
    def create_contact_types():
        ...
        with open(os.path.join(FIXTURE_DIR, "contact-types.json"), "r") as handler:
            ...
            for contact_type in data:
                ContactType.objects.get_or_create(**contact_type)
    ...
    def create_log_types():
        ...
        with open(os.path.join(FIXTURE_DIR, "data-log-types.json"), "r") as handler:
            ...
            for data_log_type in data:
                DataLogType.objects.get_or_create(**data_log_type)
    ...
    def create_datatypes():
        ...
        with open(os.path.join(FIXTURE_DIR, "datatypes.json"), "r") as handler:
            ...
            for data_type in data:
                DataType.objects.get_or_create(**data_type)
    ...
    def create_document_types():
        ...
        with open(os.path.join(FIXTURE_DIR, "document-types.json"), "r") as handler:
            ...
            for document_types in data:
                DocumentType.objects.get_or_create(**document_types)
    ...
    def create_funding_sources():
        ...
        with open(os.path.join(FIXTURE_DIR, "funding-sources.json"), "r") as handler:
            ...
            for funding_source in data:
                FundingSource.objects.get_or_create(**funding_source)
    ...
    def create_storage_resources():
        ...
        with open(os.path.join(FIXTURE_DIR, "storage-resource.json"), "r") as handler:
            ...
            for storage_resource in data:
                StorageResource.objects.get_or_create(**storage_resource)
    ...
    def create_elu_institutions():
        ...
        with open(
            os.path.join(FIXTURE_DIR, "institutions.json"), "r", encoding="utf-8"
        ) as handler:
            data = json.load(handler)
            for partner in data:
    ...
    def create_elu_cohorts():
        ...
        with open(
            os.path.join(FIXTURE_DIR, "elu-cohorts.json"), "r", encoding="utf-8"
        ) as handler:
            data = json.load(handler)
            for cohort in data:
                    ...
                    for ins in cohort["institutes"]:
                        try:
                            ...
                            raise FixtureImportError(
                                data=f"unknown partner institute {ins}"
                            )
                    c.institutes.set(institutes)
                if "owners" in cohort:
                    owners = []
                    for omb in cohort["owners"]:
                        contact_type_pi, _ = ContactType.objects.get_or_create(
        ...
        with open(handle, "r", encoding="utf-8") as f:
            ...
            for class_node in get_classes(ont):
                disease_terms.append(
                    DiseaseTerm(term_id=class_node[0], label=class_node[1])
                )
                ...
                if "type" in n_dict:
                    if ont.node_type(n_id) == "CLASS":
                        for t in n_dict["meta"]["basicPropertyValues"]:
                            if (
                                t["pred"]
                                == "http://ncicb.nci.nih.gov/xml/owl/EVS/Hugo.owl#Approved_Symbol"
                            ):
                                symbol = t["val"]
    ...
    def create_legal_basis_types():
        ...
        with open(
            os.path.join(FIXTURE_DIR, "legal-basis-types.json"), "r", encoding="utf-8"
        ) as handler:
            data = json.load(handler)
            for lbt in data:
                LegalBasisType.objects.get_or_create(**lbt)

    @staticmethod
    def create_personal_data_types():
        print("Creating GDPR-defined categories personal data.")
        with open(
            os.path.join(FIXTURE_DIR, "personal-data-types.json"), "r", encoding="utf-8"
        ) as handler:
            data = json.load(handler)
            for lbt in data:
