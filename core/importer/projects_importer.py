...
class ProjectsImporter(BaseImporter):
    ...
    def process_json(self, project_dict):
        publications = [
            ...
            for publication_dict in project_dict.get("publications", [])
        ]
        ...
        def get_project(elu_accession, title, acronym):
            if elu_accession:
                ...
                project = Project.objects.filter(title=title) | Project.objects.filter(
                    acronym=acronym
                )
                if len(project) == 1:
                    project = project[0]
                    ...
                    raise ProjectImportError(
                        data=f'Multiple projects matched by title="{title}" or acronym="{acronym}".'
                    )
                else:
        ...
        try:
            project = get_project(
                elu_accession=elu_accession, title=name, acronym=acronym
            )
            if project.is_published:
                raise ProjectImportError(
                    data=f'Updating published entity is not supported: "{project.title}".'
                )
        ...
        if project is None:
            ...
            if self.skip_on_exist:
                self.logger.warning(
                    f"Project with acronym '{acronym_to_show}' already found. It will be skipped."
                )
                return True
            self.logger.warning(
                f"Project with acronym '{acronym_to_show}' already found. It will be updated."
            )
            project.title = name
    ...
    def process_publication(publication_dict):
        ...
        if (
            publication is None
            and "citation_string" in publication_dict
            and len(publication_dict.get("citation_string")) > 0
        ):
            if (
                Publication.objects.filter(
                    citation=publication_dict.get("citation_string")
                ).count()
                == 1
            ):
    ...
    def _process_date_attribute(self, project_obj, project_dict, attribute_name):
        try:
            if (
                attribute_name in project_dict
                and project_dict.get(attribute_name)
                and len(project_dict.get(attribute_name)) > 0
            ):
                setattr(
                    project_obj,
                    attribute_name,
                    self.process_date(project_dict.get(attribute_name)),
                )
