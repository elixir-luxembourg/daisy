...
class PartnersImporter(BaseImporter):
    ...
    def process_json(self, partner_dict):
        ...
        try:
            partner = get_partner(
                elu_accession=partner_dict.get("external_id", None),
                name=partner_dict.get("name"),
            )
            if self.skip_on_exist:
                self.logger.warning(
                    f'Partner with name ""{partner_dict.get("name")}"" already found. It will be skipped.'
                )
                return True
            self.logger.warning(
            ...
            if partner.is_published:
                raise PartnerImportError(
                    data=f'Updating published entity is not supported - partner: "{partner.name}".'
                )
        ...
        partner.address = (
            partner_dict.get("address") if partner_dict.get("address") else ""
        )
