...
class Contact(CoreModel):
    ...
    first_name = TextFieldWithInputWidget(
        blank=False, verbose_name="First name of the contact"
    )

    last_name = TextFieldWithInputWidget(
        blank=False, verbose_name="Last name of the contact"
    )
    ...
    phone = TextFieldWithInputWidget(
        max_length=32, blank=True, null=True, verbose_name="Phone"
    )

    type = models.ForeignKey(
        "core.ContactType", verbose_name="Type of the contact", on_delete=models.CASCADE
    )
    ...
    def to_dict(self):
        ...
        for partner in self.partners.all():
            partners_dict.append({"acronym": partner.acronym, "name": partner.name})
