...
class Cohort(CoreTrackedDBModel):
    ...
    class AppMeta:
        help_text = (
            "Cohorts are studies that collect data and/or biosamples from a group of participants. "
            "Longitudinal, case-control, family studies are typical examples of cohorts."
        )

    ethics_confirmation = models.BooleanField(
        default=True,
        blank=False,
        null=False,
    ...
    cohort_web_page = models.URLField(
        verbose_name="Cohort" "s URL page",
    ...
    def serialize_to_export(self):
        ...
        owners = map(
            lambda v: f"[{v['first_name']} {v['last_name']}, {v['email']}]", d["owners"]
        )
