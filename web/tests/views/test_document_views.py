...
def check_document_view_permissions(
    url: str,
    user: User,
    actions_list: List[Permissions],
    ...
    if (
        user.is_part_of(DataStewardGroup())
        or (
            user.is_part_of(LegalGroup())
            and isinstance(document.content_object, Contract)
        )
...
def test_document_views_permissions(permissions, group, doc_factory, url_name, actions):
    ...
    if url_name == "document_add":
        ...
        url = reverse(
            url_name,
            kwargs={
                "object_id": document.content_object.id,
    ...
    check_document_view_permissions(
        url,
        user,
        actions,
        document,
        method="DELETE" if url_name == "document_delete" else "GET",
    )
