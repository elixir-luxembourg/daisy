...
def project_dataset_choose_type(request, pk):
    ...
    return render(
        request,
        "projects/choose_dataset_type.html",
        {
            "project": project,
        },
    )
...
def dsw_list_projects(request):
    ...
    objects = (
        Project.objects.filter(local_custodians=request.user)
        | Project.objects.filter(company_personnel=request.user)
    ).distinct()
    objects = objects.annotate(c=Count("datasets__exposures")).filter(c__gt=0)
    return render(
        request,
        "integrations/dsw/project_list.html",
        {
