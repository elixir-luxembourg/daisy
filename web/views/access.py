...
class AccessCreateView(CreateView, AjaxViewMixin):
    ...
    def has_permissions(self, user, dataset):
        return user.is_part_of(Groups.DATA_STEWARD.value) or user.can_edit_dataset(
            dataset
        )
...
class AccessEditView(CheckerMixin, UpdateView, AjaxViewMixin):
    ...
    def form_valid(self, form):
        ...
        if (
            original_object.status == StatusChoices.active
            and not self.request.user.is_part_of(Groups.DATA_STEWARD.value)
        ):
            form.add_error(
                None, "Only data stewards can edit active accesses, please contact one"
            )
        ...
        if "access_notes" not in form.changed_data:
            form.add_error(
                "access_notes", "Changes must be justified. Please update this field"
            )

        if StatusChoices.active.name in form.data.get(
            "status"
        ) and not self.request.user.is_part_of(Groups.DATA_STEWARD.value):
            form.add_error(
                "status",
                "Only data stewards are allowed to activate accesses, please contact one",
            )
